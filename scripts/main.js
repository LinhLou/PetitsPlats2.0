...
const displayRecettes = (recipes) => {
  ...
  if (recipes.length != 0) {
    ...
    const div = document.createElement('div');
    div.innerHTML = `Aucune recette ne contient "${document.querySelector('#inputSearchPrincipale').value}". Vous pouvez chercher «
...
const init = () => {
  localStorage.setItem('resultSearchPrincipale', JSON.stringify(recipes));
  displaySearchBarPrincipale();
  displayFiltre();
  displayRecettes(recipes);
  updateNbrTotalResults(recipes);
  const searchPrincipaleNode = document.querySelector('.searchBar-principale');
  const searchPrincipaleObj = new SearchPrincipaleGestion(searchPrincipaleNode, recipes);
